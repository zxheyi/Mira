...
2. 恢复案例。继续已有 Research Case 且 `caseId` 已知时，调用 `prepare_research_context`；仅把返回的 Markdown 视为截至该案例日期的已审核研究上下文，并在审核包运行记录中保留 `claimIds`、`evidenceIds` 与 `snapshotIds`。任一 ID 缺失或案例未知时，不猜测案例或补造 provenance。
