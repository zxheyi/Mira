...
const recalledMemory = curateMemory(db, {operation: "add", input: {projectId: project.id, title: "示例研究流程", content: "每次研究先核对来源。", kind: "convention", source: "manual", confidence: 1, importance: 5}}, authorizeCuration(db, project.id, {scopes:["memory.review","memory.mutate","research.review","research.mutate","recall.feedback","context.delivery"],actor: "test", reason: "Synthetic UI fixture"}));
const lifecycleMemory = curateMemory(db, {operation:"add",input:{projectId:project.id,title:"生命周期示例",content:"保留普通记忆管理验收。",kind:"fact",source:"manual",confidence:1,importance:5}},authorizeCuration(db,project.id,{scopes:["memory.review","memory.mutate","research.review","research.mutate","recall.feedback","context.delivery"],actor:"test",reason:"Synthetic lifecycle fixture"}));
...
try {
  ...
  await nav("召回审计");
  const recallCard = page.locator("article").filter({hasText:"示例研究流程"}).filter({has:page.getByRole("button",{name:"标注召回",exact:true})});
  await recallCard.getByRole("button",{name:"标注召回",exact:true}).click();
  await page.locator('[data-injected-memory="' + recalledMemory.id + '"]').selectOption("relevant");
  await page.getByLabel("总体结果").selectOption("partial");
  await page.getByLabel("搜索未注入的 active Memory").fill("待核对的证据规则");
  await page.locator("[data-missing-memory]").check();
  await page.getByLabel("用户反馈原因").fill("核心流程有用，但证据规则没有被召回。");
  await page.screenshot({path:join(root,"desktop-recall-feedback.png"), fullPage:true});
