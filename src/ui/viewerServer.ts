...
<body>
  ...
  <script>
    ...
    async function renderJobs() {
      ...
      document.getElementById('jobs').innerHTML = '<h2>轮次处理进度</h2>' + workflows.map(item=>'<article class="panel"><b>' + escapeHtml(item.stage) + '</b><p>' + escapeHtml(item.turnId) + '</p><p>轮次来源：' + escapeHtml(item.turnProvenance?.identitySource || 'unknown') + ' · 捕获路径：' + escapeHtml(item.turnProvenance?.capturePath || 'unknown') + '</p><p>' + escapeHtml(item.nextAction) + '</p><details><summary>阶段明细</summary><pre>' + escapeHtml(JSON.stringify(item,null,2)) + '</pre></details></article>').join('') + '<h2>后台任务</h2><p class="muted">Outbox 使用租约恢复；CLI outbox run --drain 处理事实提交后的可靠跟进。</p><h2>Domain Outbox</h2>' + (messages || '<div class="empty">暂无 Outbox 消息</div>') + '<h2>Distill Jobs</h2>' + (distill || '<div class="empty">暂无提炼任务</div>');
    }
