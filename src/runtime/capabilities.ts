...
export const CAPABILITY_SCOPES=['memory.review','memory.mutate','research.review','research.mutate','recall.feedback'] as const;
export type CapabilityScope=typeof CAPABILITY_SCOPES[number];
export type CapabilityPolicy={actor:string;reason:string;scopes?:CapabilityScope[]};
...
export function allows(policy:CapabilityPolicy|undefined,scope:CapabilityScope):boolean {
  return !!policy && (policy.scopes===undefined || policy.scopes.includes(scope));
}
