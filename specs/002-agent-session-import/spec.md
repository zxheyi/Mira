...
P0 起步覆盖 Markdown / 纯文本会话文件导入，后续已扩展为支持 JSONL transcript：

- Codex Markdown 会话摘要导入。
- Claude Code Markdown 会话摘要导入。
- 通用 Markdown 会话导入。
- Codex JSONL transcript 导入。
- Claude Code JSONL transcript 导入。
- 导入后保存为当前项目的 Thread。
...
- `mira import --source <source> --format jsonl --path <file>` 应 normalize transcript 为 Markdown Thread 文本。
- `source` 必须是 `codex`、`claude-code` 或 `markdown`。
- Markdown 导入的 `rawFormat` 为 `markdown`。
- JSONL 导入的 `rawFormat` 为 `jsonl`，保存的 `rawText` 为可读 Markdown。
- 如果用户提供 `--id`，导入使用该 id。
...
- Codex / Claude Code JSONL transcript 可以通过 CLI 导入并保存为可 distill/search 的 Thread。
