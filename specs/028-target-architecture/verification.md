...
| Investment Research Context is separate from generic memory context | `src/research/researchContext.ts`; Briefing receives only aggregate Research status; MCP delegates to the same read-only projection | Research Context and MCP tests prove only active approved Claims with current verified supporting Evidence are included, while draft/stale research stays excluded and unverified counter-evidence remains visible as omitted |
