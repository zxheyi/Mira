...
Phase 5: schema v16 adds candidate provenance and acceptance mode. The stored transcript must uniquely attribute an excerpt to a user message for automatic acceptance; assistant/tool/unattributed evidence remains pending even at confidence 1. Original policy reasons are immutable and separate from review reason. Legacy provenance and acceptance are unknown. CLI/MCP/Viewer support candidate pagination; Viewer defaults to pending review and links accepted candidates to immutable Memory correction/archive operations.

Phase 6: MCP --profile core exposes common lifecycle/proposal/context tools; research adds the Research tools; admin/full retain all tools. Default full preserves existing configurations. Profiles never grant authorization. `mira workflow [--turn ID]`, `get_workflow_progress` and Viewer background tasks show durable capture, Outbox, linked distillation jobs and candidate state, including source changes and errors. after_turn.processing reports captured/queued, not accepted; stable duplicate finish calls reuse the same outbox IDs. Native Codex/Claude/Cursor adapters remain session-granularity integrations.

Phase 7: real stdio/CLI/HTTP acceptance and v14-to-v16 migration tests cover the combined contract. Context generation/recall runs inside a database transaction; host delivery acknowledgements are transactionally idempotent. Status reads expose registered scope; packet freshness separates Briefing state from Research case dates. Quoted role labels cannot claim user attribution. See context-trust-validation.md for commands, browser checks and operational boundaries.
